...
export default class TextureAtlas extends TextureBase<'image'> {
  ...
  constructor(
    ...
    super(
      'image',
      'atlas',
      scene.loader.textureStack.find((image) => image.key === textureKey)!.value
        .texture,
      w,
      h
    );
    ...
    if (this.json) {
      if ('name' in this.json && 'data' in this.json) {
        if (typeof this.json.name !== 'string') {
          new Debug.Error('Invalid TextureAtlas JSON. "name" must be a string');
          return;
        }
        if (!Array.isArray(this.json.data)) {
          new Debug.Error('Invalid TextureAtlas JSON. "data" must be an array');
          return;
        }
      } else {
        new Debug.Error(
  ...
  public get(key: string): Texture<'image'> | undefined {
    return this.scene.loader.textureStack.find((texture) => texture.key === key)
      ?.value;
  }
}

...
export default class Game {
  ...
  constructor(config: Duck.Types.Game.Config) {
    ...
    this.eventEmitter.on(EVENTS.GAME.CONTEXT_LOST, () => {
      ...
      this.eventEmitter.emit(EVENTS.GAME.CONTEXT_RESTORED, this.renderer.ctx);
    });
    ...
    this.scenes = {
       ...
       */
      remove: (scene: Scene) => {
        const f = this.stack.scenes.find((_scene) => _scene.key === scene.key);
        if (f) {
          this.stack.scenes.splice(this.stack.scenes.indexOf(scene), 1);

...
export default class Mouse {
  ...
  constructor(
    ...
    if (mouseUp) {
      this.mouseInput.eventEmitter.on(`MOUSE_UP_${this.descriptor}`, mouseUp);
    }
  ...
  protected registerListeners() {
    document.addEventListener('mousedown', (e) => {
      if (e.button === this.button) {
        this.mouseInput.eventEmitter.emit(`MOUSE_DOWN_${this.descriptor}`, e);
      }
    });
    document.addEventListener('mouseup', (e) => {
      if (e.button === this.button) {
        this.mouseInput.eventEmitter.emit(`MOUSE_UP_${this.descriptor}`, e);
      }
    });
    document.addEventListener('mousemove', (e) => {
      if (e.button === this.button) {
        this.mouseInput.eventEmitter.emit(`MOUSE_MOVE_${this.descriptor}`, e);
      }
    });
  }
}

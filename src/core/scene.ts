...
export default class Scene extends Render {
  ...
  public add: {
    ...
    gameobject: {
      ...
      rect: (x: number, y: number, w: number, h: number, color: Color) => Rect;
  ...
  constructor(key: string, game: Game, visible?: boolean) {
    ...
    this.add = {
      ...
      map: {
        tilemap: (
          ...
          const myTileMap = new TileMap(origin, tilelayers, this.game, this);
        ...
        tileLayer: (
          ...
          return new TileLayer(tileset, map, this.game, this, zIndex, visible);
        },
      },
      effect: (
        rangeX: Duck.Types.ParticleEmitter.Range,
        ...
        const myEffect = new Effect(rangeX, rangeY, particleEmitter, this.game);
    ...
    this.tools = {
      ...
      physics: {
        rectToRectIntersect: (rect: Rect | Sprite, rect2: Rect | Sprite) => {
